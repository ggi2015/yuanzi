...
高度平衡  
平衡因子(BalanceFactor)：将二叉树上结点的左子树深度减去右子树深度的值，只能为-1，0，1；
...
### 多路查找树（B树）
multi-way search tree，其每个结点的孩子数可以多于两个，且每个结点处可以存储多个元素。
- 2-3树
其中每个结点都有两个孩子（2结点）或三个孩子（3结点）
一个2结点包含一个元素和两个孩子（或没有孩子），且与二叉排序树类似。（这个2结点要么没有孩子，要么有2个孩子，不能没有孩子）；3结点同理，从左到右，由小到大； 
2-3树复杂在于新结点的插入和已有结点的删除。  
2-3树插入实现
2-3树删除实现
...
一个m阶B树具有如下属性：
   ...
   - 每一个。。。[dahuashujujiegou_P375]  

- B+树
B+树是应文件系统所需而出的一种B树的变形树。在B+树中，出现在分支结点中的元素会被当作他们在该分支结点位置的中序后继者（叶子结点）中再次列出。另外，每一个叶子结点都会保存一个指向后一个叶子结点的指针。  
一棵m阶B+树和B树的差异在于：
   - [dahuashujujiegou_P378]

### 散列表查找
通过某个函数f，使得： `存储位置=f(关键字)`， 通过查找关键字不需要比较就可以获得需要的记录的存储位置，该存储技术即散列技术。  
散列技术是在记录的存储位置和他的关键字之间建立一个确定的对应关系f，使得每个关键字key对应一个存储位置f(key)。查找时，已知key，则记录一定在f(key)的位置上。  
这种对应关系f称为散列函数，又称哈希(Hash)函数。采用散列技术将记录存储在一块连续的存储空间中，这块连续存储空间称为散列表或哈希表(HashTable);
散列地址：关键字对应的记录存储位置；  
散列表查找步骤：
   - 在存储时，通过散列函数计算记录的散列地址，并按照此散列地址存储该记录；  
   - 当查找时，通过同样的散列函数计算记录的散列地址，按此散列地址访问该记录。  
散列技术既是一种存储方法，也是一种查找方法； 散列主要是面向查找的存储结构。  
散列技术最适合的求解问题是查找与给定值相等的记录。 不适合次关键字，范围查找 。  

### 散列函数的构造方法  
两个原则：计算简单（散列函数计算时间不超过其他查找技术的比较时间），散列地址分布均匀（存储空间有效利用，并减少处理冲突而耗费的时间）；  
- 直接定址法  
f(key)=key  或者 f(key)=a*key+b(a、b为常数)
简单均匀无冲突，但要事先知道关键字分布情况，适合查找表小且连续的情况。  
- 数字分析法  
抽取：使用关键字的一部分来计算散列存储位置的方法。
适用于关键字位数较大的情况，如果事先知道关键字分布且其若干位分布较均匀，可用。  
- 平方取中法  
适用于不知道关键字分布，而位数又不是很大的情况。  
- 折叠法  
将关键字从左到右分割成位数相等的几部分（最后一部分位数不够可以短些），然后将这几部分叠加求和，并按散列表表长，取后几位作为散列地址。  
不需要知道关键字的分布，适合关键字位数较多的情况。  
- 除留余数法
对于散列表长为m的散列函数公式为：`f(key) = key%p (P<=m)`， 不仅可以直接mod，亦可在折叠、平方取中后再取模；   
关键在于选p，p选的好，不会有同义词；根据前辈经验，若散列表表长为m，通常p为小于或等于表长（最好接近m）的最小质数或不包含小于20质因数的合数。  
- 随机数法
` f(key) = random(key)`  
当关键字长度不定时，该方法合适。

根据不同情况采用不同散列函数需要考虑的因素：
   - 计算散列所需的时间
   - 关键字的长度
   - 散列表的大小
   - 关键字的分布情况
...
### 处理散列冲突的方法 
当`key1！=key2`，但`f(key1)=f(key2)`;
- 开放定址法
一旦发生冲突，就去寻找下一个空的散列地址，只要散列表足够大，空的散列地址总能找到，并将记录存入。
即`fi(key) = (f(key) + di) % m (di = 1,2,3...,m-1)  
线性探测法  
堆积  
二次探测法，即`fi(key) = (f(key) + di) % m (di=1^2,-1^2,2^2,-2^2,...,q^2,-q^2,q<=m/2)`  
随机探测法，即在冲突时，对于位移量di采用随机函数计算得到，`fi(key) = (f(key) + di) % m (di是一个随机数列);  
总之，开放定址法只要在散列表未填满时，总是能够找到不发生冲突的地址。  
- 再散列函数法  
对于散列表，事先准备多个散列函数`fi(key) = RHi(key) (i=1,2,...,k)`，此处RHi就是不同的散列函数，可以把之前除留余数、折叠、平方取中等方法全用上。当发生散列地址冲突，换一个散列函数计算即可。
不产生聚集，增加计算时间。  
- 链地址法  
将所有关键字为同义词的记录存在一个单链表中，这种表为同义词子表，在散列表中只存储所有同义词子表的头指针。无论有多少冲突，只是在当前位置给单链表增加结点的问题。  
但有遍历单链表的性能损耗。  
- 公共溢出区法  
给所有冲突的关键字建立一个公共的溢出区来存放。  
查找时，对给定值通过散列函数计算出散列地址后，先与基本表的相应位置进行对比，如果相等，则查找成功；如果不成功，则到溢出表进行顺序查找。  
...
对于内排序，影响排序算法的性能因素：
- 时间性能：主要两种操作，比较和移动；高效的内排序算法应该是具有尽可能少的关键字比较次数和尽可能少的记录移动次数。  
- 辅助空间
- 算法的复杂性  
...
基本思想：通过n-i次关键字间的比较，从n-i+1个记录中选出关键字最小的记录，并和第i个(1<=i<=n)个记录交换之。  
...
基本操作：将一个记录插入到已经拍好序的有序表中，从而得到一个新的、记录数增1的有序表。  
...
跳跃分割的策略：将相距某个“增量”的记录组成一个子序列，这样才能保证在子序列内分别进行直接插入排序后得到的结果时基本有序而不是局部有序的。
...
堆排序算法  
基本思想：将待排序的序列构造成一个大顶堆。此时，整个序列的最大值就是堆顶的根结点。将他移走（其实就是将其与堆数组的末尾元素互换，此时末尾元素就是最大值），然后将剩余的n-1个序列重新构造成一个堆，这样就会得到n个元素中的次小值。如此反复执行，便能够得到一个有序序列。  
...
### 归并排序
基本思想：假设出使序列含有n个记录，则可以看成n个有序的子序列，每个子序列的长度为1，然后两两归并，得到[n/2]([x]表示不小于x的最小整数)个长度为2或1的有序子序列；再两两并归，.....，如此反复，直至得到一个长度为n的有序序列为止，这种排序方法称为2路并归排序。
